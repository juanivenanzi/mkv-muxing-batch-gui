...
class MuxSettingTab(QWidget):
    ...
    def setup_mux_tools_layout_first_row(self):
        self.h1 = QHBoxLayout()
        self.l1 = QLineEdit()
        self.l1.setPlaceholderText("Nuevo sufijo")
        self.c1 = QCheckBox("")
    ...
    def setup_clear_job_queue_button(self):
        self.clear_job_queue_button.setText("Limpiar todo")
    ...
    def open_select_destination_folder_dialog(self):
        ...
        if temp_folder_path == "" or temp_folder_path.isspace():
            ...
            invalid_dialog = InvalidPathDialog(
                error_message="Algunos videos de origen y destino están en la misma carpeta",
                parent=self,
            )
        ...
        for video_file in GlobalSetting.VIDEO_FILES_ABSOLUTE_PATH_LIST:
            if not Path.is_file(Path(video_file)):
                invalid_dialog = FileNotFoundDialog(
                    window_title="Archivo no encontrado",
                    error_message='Archivo: "' + video_file + '" no encontrado',
                    parent=self,
                )
                invalid_dialog.execute()
                return False
